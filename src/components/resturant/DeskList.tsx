...
@Component({
    computed: {
        ...mapGetters({
            deskList: 'resturant/getDeskList',
            lastPage: 'resturant/lastDeskPage',
            current: 'resturant/currentDeskPage',
        })
    },
    methods: {
        ...mapActions({
            fetch: 'resturant/deskFetch',
            nextPage: 'resturant/nextDeskPage',
            prevPage: 'resturant/prevDeskPage',
            goto: 'resturant/DeskGotoPage',
            search: 'resturant/deskSearch',
        })
    }
})
    ...
    render(): VNode {
        return <div class="desk__list">
            ...
            <div class="flex items-center space-x-2">
                <input type="search" placeholder="Search Desks" v-model={this.searchText} class="bg-gray-700 appearance-none relative block w-56 px-3 py-2 placeholder-gray-500 outline-none text-gray-400 border border-transparent rounded-md sm:text-sm ml-auto" />
            ...
            {!this.isLoading ? [<div class="md:space-y-1 pt-3">
                <div class="md:space-y-1">
                    {this.deskList.map((desk: iDesk, index: number) => (<DeskCard desk={desk} key={desk.id} style={{ '--transition-delay': index * 0.3 + 's' }} />))}
                </div>
            </div>,
            ...
            <Modal v-model={this.showForm}>
                <DeskCreate onClose={() => { this.showForm = false }} />
            </Modal>
        </div>
    }
